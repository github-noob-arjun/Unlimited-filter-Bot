...
@Client.on_message(filters.command(["add", "filter"]) & filters.user(Config.AUTH_USERS))
...
@Client.on_message(filters.command(["delfilter", "stop", "del"]) & filters.user(Config.AUTH_USERS))
...
@Client.on_message(filters.command(["delall", "stopall"]) & filters.user(Config.AUTH_USERS))
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            SMS = await message.reply_text(reply_text, disable_web_page_preview=True)
                            await asyncio.sleep(600)
                            await SMS.delete()
                        else:
                            button = eval(btn)
                            SMS =await message.reply_text(
                                reply_text,
                        ...
                        if btn == "[]":
                            SMS = await message.reply_cached_media(
                                fileid,
